...
class Listener(StreamListener):
    def on_update(self, status):
        try:
            ...
            a_toot = {}
            count = cnt['count']
            if f'{save_db}_{count//1000000}' in server:
                db = server[f'{save_db}_{count//1000000}']
            elif count % 1000000 == 0:
                db = server.create(f'{save_db}_{count//1000000}')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 8:
        raise ValueError(
            "[admin, pssword, access_token, base_url, cluster_nodes, save_db, port] are needed! Quiting")

    # admin = 'admin'
    # password = 'password'
    # access_token = 'token'
    # base_url = f'https://aus.social'
    # cluster_nodes = "address1_address2_address3"
    # save_db = "mastodon_2"
    # port = 5984
    admin = sys.argv[1]
    password = sys.argv[2]
    ...
    base_url = sys.argv[4]
    cluster_nodes = sys.argv[5].split("_")
    save_db = sys.argv[6]
    port = sys.argv[7]
    ...
    node_servers = [
        f"http://{admin}:{password}@{node_ip}:{port}/" for node_ip in cluster_nodes]
