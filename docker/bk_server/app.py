...
node_servers = [
    f"http://{admin}:{password}@{node_ip}:5984/" for node_ip in cluster_nodes]
...
def api_0(state, scenario):
    ...
    db = server['twitter1']
    view = db.view(f'{scenario}/new-view',
                   startkey=[state], endkey=[state, {}], group_level=2)
...
def api_sudo_unemployment(state):
    if state == 'newsouthwales':
        ...
        state = "western australia"
    db = server['sudo']
    view = db.view('unemployment/new-view',
                   startkey=[state], endkey=[state, {}])
...
def generate_color(df):
    ...
    for i in bottom_3:
        bottom_3_final.append(
            df_get_rank[df_get_rank['index'] == i].index.item())
...
def create_figure(state, twi, sudo):
    ...
    df = twi_df.merge(sudo_df, how='inner', on='sua').sort_values(
        'value_official', ascending=False)
    ...
    bars1 = ax1.bar(primary_bar_positions,
                    df['value_official'], width=bar_width, color='silver')
    ...
    if state == 'australian capital territory' or state == 'offshore territories' or state == 'northern territory':
        ...
        bars2 = ax2.bar(secondary_bar_positions,
                        df['value_media'], width=bar_width, color='dimgrey')
    else:
        bars2 = ax2.bar(secondary_bar_positions,
                        df['value_media'], width=bar_width, color=colors)
...
def api_twi_state_total(topic, state):
    ...
    for row in view:
        if row['key'] == state:
            geo[state] = {'lat': row['value']
                          ['lat'], 'lng': row['value']['lng']}
            break
...
def api_page2_sudo(state, topic):
    ...
    if topic == 'agism':
        for row in view:
            ...
            results.append(result)
            rank.append(
                [row['value']['ageing_population_percentage'], row['key'][1]])
...
def api_sudo_agism_state_city(state):
    ...
    data = []
    for row in view:
        result = {"city": row['key'][1], 'count': row['value']['ageing_population'],
...
def api_sudo_sexism_state_city(state):
    ...
    data = []
    for row in view:
        result = {"city": row['key'][1], 'male': row['value']['males'],
                  'female': row['value']['females'], 'ratio': row['value']['gender_ratio']}
        data.append(result)
...
def api_sudo_unemployment_state_city(state):
    ...
    data = []
    for row in view:
        result = {"city": row['key'][1], 'employed': row['value']
                  ['people_employed'], 'employment_rate': row['value']['employment_rate']}
        data.append(result)
...
if __name__ == '__main__':
    ...
    host_add = sys.argv[1]
    port_add = sys.argv[2]
    app.run(debug=True, host=host_add, port=port_add)
